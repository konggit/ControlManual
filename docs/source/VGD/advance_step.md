...
自锁功能:是按下按钮不会马上弹起,带”自锁“ 功能的按钮可以设置接收命令来设置是否按下或抬起状态  
![自锁功能](../images/steps/button_lock.png "自锁功能")  
 图中表示 收到数据 ”1ebeef7302002abe0101“ 按钮会自动按下[接收后按下]  
          收到数据 ”1ebeef730200ea7f0100“ 按钮会自动弹起[接收后抬起]  
  #### 注意这里的十六进制数据用小写字母，不带空格
  这个功能可以用一个定时器发一个查询命令，来同步不同客户端的一些按钮状态 
...
- 接收数据头：设备返回的命令头部数据(参数变量前面的数据) 
  ...
  注意这里只解 4141 开始的 数字的 最后两个字节, 比如 设备发过来是 AABB4141643200 解出的数据是3200 值等于0x0032

  
![电平接收数据](../images/steps/advance_16.png "电平接收数据")  
  改进 20250724：
  增加 数据位置 功能(当设备发一条指令数据里含有多个参数值的时候可以使用这个功能)
  如果 数据位置 为0 则功能同上,
  如果 数据位置 不为0 则表示 使用 “接受数据头” 数据位后的第几位开始的数据。
  且有 起始值 结束值 来决定 使用 无符号数据还是有符号数据，是一个字节还是两个字节。
![电平接收数据](../images/steps/advance_23.png "电平数据位置")
## 设备状态
  只是用于检测设备是否在线
...
## 提取器
  用于收到某个数据后触发其他控件(鼠标点击按钮的动作)  
  例子:  
  1. 拖入 按钮 设置 按下和抬起 命令数据  
  ![按钮](../images/steps/advance_20.png "按钮控件")  
  按下发送 BE EF 56 02 00 00 00 01 00  
  抬起发送 BE EF 56 02 00 00 00 01 01  
  ![提取器](../images/steps/advance_21.png "提取器")  
  接收数据：1234 这里表示收到提取器绑定的设备(这里是本地电脑)发来12 34 数据时候 触发这个提取器(**这里只要发过来数据里包含了1234就会触发**)  
  触发控件： 按钮2 这里触发按钮2 的 点击事件, 就是会发送按钮2的 按下 和抬起 来两条命令。  
  接收时发送： 1122 , 提取器本身在收到 1234 数据后会 发送 1122 输出给 设备("本地电脑”)  
  测试结果:  
  ![提取器](../images/steps/advance_22.png "提取器")    
